...
export const ShippingPaymentMethod = () => {
  const [selectedItem] = useState(
    MOCK_PAYMENT_METHODS.find((item) => item.isDefault),
  );
  ...
  return (
    <div className="flex flex-col gap-5 lg:flex-row lg:gap-10 pb-6 border-b border-bo-primary">
      <section className="w-full lg:max-w-100 animate-slide-up">
        ...
        <Select className="**:text-sm" defaultValue="Select Payment Method">
          <SelectButton className="btn btn--border max-w-full! justify-between px-4" />
          <SelectContent className="w-full bg-b-primary border border-bo-primary rounded-lg overflow-hidden mt-1 animate-slide-down">
            {MOCK_PAYMENT_METHODS.map((item) => (
              <SelectItems
                ...
                key={item.id}
                onClick={() => {}}
                className="flex justify-between items-center h-12 hover:bg-b-secondary cursor-pointer px-4 capitalize"
              >
